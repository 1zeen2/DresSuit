...
import com.sist.web.dao.DsMemberDAO;
import com.sist.web.dto.ApiResponse;
import com.sist.web.dto.DsMemberDTO;
import com.sist.web.entity.DsMemberEntity;
import com.sist.web.exception.DuplicateMemberException;
import com.sist.web.exception.PwdCharException;
import com.sist.web.exception.ValidatorException;
		...
		try {
			...
			if (!password.matches(passwordRegex)) {
		        if (!password.matches(".*[a-zA-Z].*")) {
					exceptions.add(new PwdCharException("영문자"));
				}
		        if (!password.matches(".*[0-9].*")) {
					exceptions.add(new PwdCharException("숫자"));
				}
		        if (!password.matches(".*[!@#$%^&*()_+\\-=\\[\\]{};':\"\\\\|,.<>\\/?].*")) {
					exceptions.add(new PwdCharException("특수문자"));
				}
		    }
